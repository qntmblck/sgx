export default function BeneficiosTecnologicos() {
  ...
  return (
    <section id="beneficios" className="relative py-16 px-4 sm:px-6 lg:px-12 max-w-full overflow-hidden">
      ...
      {/* Fondo translúcido SGX (estilo HeroSection) */}
      <div className="absolute inset-0 px-2 sm:px-4">
        <div className="w-full h-full rounded-[48px] backdrop-blur-md shadow-2xl ring-1 ring-inset ring-white/20 bg-gradient-to-br from-white/60 via-lime-100/40 to-white/20" />
      </div>
      ...
      <div className="relative z-10 w-full overflow-x-auto scrollbar-hide">
        ...
        >
          {[...beneficios, ...beneficios].map((item, idx) => (
            <div
              key={idx}
              className="flex flex-shrink-0 w-[300px] sm:w-[340px] h-auto p-5 rounded-xl shadow-lg hover:scale-[1.01] transition-transform duration-300
